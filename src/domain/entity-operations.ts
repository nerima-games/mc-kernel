...
export const spawnEntity = <S>(
  roster: EntityRoster<S>,
  request: SpawnRequest<S>,
): SpawnOutcome<S> => {
...
export const changed = <S>(state: EntityState<S>): EntityTransition<S> => ({
  _tag: 'Changed',
  state,
})
...
export const normaliseRoster = <S>(
  ...
  const incoming: ReadonlyArray<Entity<S>> =
    typeof roster === 'object' && roster !== null && Array.isArray(roster.entities)
      ? roster.entities
      : NO_ENTITIES
  const savedSerial =
  ...
  incoming.forEach((entity) => {
    ...
    entities.push({
      ...
      behaviour:
        repairBehaviour === undefined
          ? repaired.behaviour
          : repairBehaviour(entity.kind, repaired.behaviour),
    })
  })
