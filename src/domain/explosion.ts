import { DEFAULT_EXPLOSION_LIMITS } from './explosion-data.js'
import type {
  ExplosionBlock,
...
const entityExposure = (
  ...
  for (const height of heights) {
    ...
    if (result.loaded && result.attenuation < 1) {
      visible += 1
    }
  }
...
const planDestroyedBlocks = (
  ...
  let readBlock: ExplosionBlockReader = blocks
  if (sharedTrace) {
    readBlock = sharedTrace.readBlock
  }
          ...
          if (visitedBlocks >= limits.maxVisitedBlocks) {
            ...
            if (block && block.destructible) {
              ...
              if (!ray.loaded) {
                ...
                const blast =
                  radius * (BLAST_BASE_FACTOR + hashUnit(seed, x, y, z) * BLAST_RANDOM_FACTOR) -
                  distance -
                  ray.attenuation
    ...
    if (entity) {
      ...
      if (radius > 0 && distance <= radius) {
        ...
        if (exposure > 0) {
          ...
          let direction: Position = { x: 0, y: 0, z: 0 }
          ...
          entityEffects.push({
            damage:
              (impact * impact + impact) * ENTITY_DAMAGE_QUADRATIC_FACTOR * ENTITY_DAMAGE_RADIUS_FACTOR * radius +
              ENTITY_DAMAGE_BASE,
            exposure,
            id: entity.id,
...
export const applyExplosionPlan = (plan: ExplosionPlan, commit: ExplosionCommit): void => {
  const mutation: ExplosionMutation = {
