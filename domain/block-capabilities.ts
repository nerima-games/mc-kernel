/**
 * Block capability FLAGS — the boolean half of the block behaviour model.
 *
 * The authoritative source for which capabilities exist, what type each one
 * must have, and what each one must default to, is
 * `docs/capability-flag-audit.md` (28 rows in its §3 table; its §7 prose says
 ...
 * plan.md §3.1 listed as booleans are NOT booleans:
 *
...
/**
 * THE flag table. Adding a flag = adding one line here. Nothing else in this
...
export const BLOCK_CAPABILITY_DEFAULTS = {
   ...
   * plan.md §3.12 wants this in kernel; the reference had it as a local constant.
   */
   ...
   * is non-opaque (audit §4.7 checked this explicitly and concluded derivation
   * is impossible). The audit does note that `passable === true` should imply
...
) as ReadonlyArray<BlockCapabilityFlag>
 ...
 *
 * Unknown keys in `overrides` are ignored rather than rejected, which is what
 * lets a repository pinned to an older kernel keep loading data authored
 * against a newer one.
 */
...
export const capabilityOf = (overrides: BlockCapabilityOverrides, flag: BlockCapabilityFlag): boolean =>
  overrides[flag] ?? BLOCK_CAPABILITY_DEFAULTS[flag]
