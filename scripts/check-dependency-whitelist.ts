 ...
 *
 * ---------------------------------------------------------------------------
 * For mc-kernel the dependency set is EMPTY, and that emptiness is the whole
 * point. Kernel is the root of the four-tier architecture: every other
 * repository may import it, so kernel importing any of them would create a
 * cycle by construction. Its only dependency is `effect`, which is not an org
 * package and is therefore outside this script's remit.
 *
 * The other 15 rows below are mirrored from plan.md §2.1 so that this copy of
 * the gate can actually detect a cycle and can actually explain a
 * transitive-closure violation. They are NOT import licences for mc-kernel:
 * only kernel's own (empty) row grants those.
 * ---------------------------------------------------------------------------
 */
export const REPOSITORY_POLICY = {
  thisPackage: '@nerima-games/mc-kernel',

  // The full 16-repository roster, transcribed from plan.md §2.1.
  // Tier 1 (stable libraries): kernel, noise, meshing, physics, save, audio
  // Tier 2 (foundation, 名詞):  worldgen, sim, render, playground-kit
  // Tier 3 (experience, 動詞):  mx-gameplay, mx-redstone, mx-ui, mx-multiplayer
  // Tier 4 (composition):       mc-compose
  // Plus mc-dev-meta, the workspace that clones the others (plan.md §6 Step 0).
  dependencyGraph: new Map<string, ReadonlySet<string>>([
    // --- Tier 1: stable libraries. Kernel-only, hence mutually independent and
    // buildable in parallel (plan.md §2.2).
    ...
    // --- Tier 2: foundation. State and services.
    ...
    // Dev tooling, but its OWN dependencies are ordinary runtime edges.
    [
      '@nerima-games/mc-playground-kit',
      new Set(['@nerima-games/mc-worldgen', '@nerima-games/mc-sim', '@nerima-games/mc-render']),
    ],

    // --- Tier 3: experience modules. Rules and UI. Note the absence of any
    // edge BETWEEN them: plan.md §2.3-1 requires experience modules to talk
    // only through foundation services, never to each other.
    [
    ...
    // --- Tier 4: composition. Layer merge + the single total stage order.
    [
    ...
    // --- The meta workspace. It clones the other repositories into `repos/`
    // and binds them with `workspace:*`; it depends on none of them by name.
