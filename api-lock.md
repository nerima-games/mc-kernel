...
### BlockPositionKey  `type`

```ts
type BlockPositionKey = string & Brand.Brand<'BlockPositionKey'>;
```
...
### blockPositionOfKey  `const`

```ts
const blockPositionOfKey: (value: BlockPositionKey) => BlockPosition;
```
...
### decodeBlockPositionKey  `const`

```ts
const decodeBlockPositionKey: (value: string) => BlockPosition | undefined;
```
