/* eslint-disable max-statements, no-magic-numbers -- Vanilla enchantment compatibility is a finite contract matrix. */
...
describe('supported vanilla enchantment data', () => {
  ...
  it('narrows only registered enchantment ids', () => {
    ...
    expect(Reflect.apply(enchantmentRuleFor, undefined, ['not_registered'])).toBeUndefined()
  })
  ...
  it('exposes target compatibility and conflicts from the rule data', () => {
    ...
    expect(Reflect.apply(enchantmentAppliesTo, undefined, ['not_registered', 'iron_sword'])).toBe(false)
    ...
    expect(Reflect.apply(enchantmentsConflict, undefined, ['not_registered', 'sharpness'])).toBe(false)
    expect(Reflect.apply(enchantmentsConflict, undefined, ['sharpness', 'not_registered'])).toBe(false)
  })
