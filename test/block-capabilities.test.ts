...
describe('block capability flags — the additive-safety guarantee', () => {
  it.effect(
    'a block definition that omits a flag resolves to that flag documented default, which is what makes adding a flag a semver-MINOR change across all 16 repositories',
    () =>
      Effect.sync(() => {
  ...
  it.effect(
    ...
    () =>
      Effect.sync(() => {
        ...
        // docs/capability-flag-audit.md §4.6-§4.9 shows it is wrong: the
...
describe('the three capabilities plan.md typed wrongly (audit §5)', () => {
  it.effect('`emissive`, `transparent` and `fluid` are NOT boolean flags', () =>
    Effect.sync(() => {
      // Regression guard. plan.md §3.1 lists these as booleans; the audit
  ...
  it.effect('the four capabilities plan.md typed correctly are still boolean flags', () =>
    Effect.sync(() => {
      // plan.md §3.1 named seven; four of them survive as booleans unchanged.
