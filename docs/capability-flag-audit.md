...
1. `packages/core/domain/block-type.ts:3-132` の `BlockTypeSchema` から参照実装の語彙を取得。**リテラル数 120**(plan.md の記述は概数)。
   - **2026-07-27 追記: この参照実装の 120 は再計数して確認済み。**`Schema.Literal(...)`(`block-type.ts:4-131`)は
...
という点で記録する価値がある。§2-1 の「参照実装のリテラル数 120」は再計数して正しいことが
確認されている（`docs/testing.md` §5.2）。現行 kernel の `BlockType` は、そこに kernel 固有の
3 行を加えた 123 行である。
...
### 4.10 追記(2026-07-27): 参照実装の 120 語彙に対応して見つかった §4.9 の追加事例
