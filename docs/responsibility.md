...
| Biome                   | Java Edition 1.21 の biome の閉じた語彙と、公式データが**保持している**気温・降水量・降水種別・草/葉/水の tint・ディメンション適合。値は misode/mcmeta の `1.21-data` タグと突き合わせてある。vanilla が**生成時に導出する**もの（`temperature_modifier` による凍結、`grass_color_modifier` による swamp の草色）は模さない——`deep_frozen_ocean` の気温が 0.5 で `frozen_ocean` と降水種別が非対称になるのはそのため。生成・分布・noise パラメータは `mc-worldgen`、tint の描画は `mc-render` |
