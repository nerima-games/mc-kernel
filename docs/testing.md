...
| `drops` が黙って何も落とさない | 18 行 | 「自分自身を落とす」と書きながらアイテム形を持たない行。うち 7 行はアイテム形を与えて解消し、`cobweb` は本来の上書き（`string`）を転記し、残る 10 行は §5.3.5 の理由で `DROPS_NOTHING` を明示した |
...
### 5.3.5 意図的に保留した 10 リテラル（条件 4 の対象外だが、記録する）

**ブロック語彙は 120 で完成しているが、アイテム語彙は規則どおりなら本来あと 10 多い。**
`sapling` / `dandelion` / `poppy` / `brown_mushroom` / `red_mushroom` / `tall_grass` /
`fern` / `sugar_cane` / `cactus` / `lily_pad` は §5.3.4 の規則を満たす（参照実装は
いずれも自分自身を落とす）が、**入れていない**。

理由は kernel の中に無い。`PlaceableItemType = ItemType & BlockType` なので、
**アイテム形を与えることは「置ける」を意味する**。mx-gameplay の
`test/place-block.test.ts` は F7 として次を記録している —— `block-support.ts:75-91` は
この 10 種に*個別の* `SUPPORT_RULES` を与えているのに、mx-gameplay は 10 種すべてを
フォールバック（`canSupportAttachments`）で答えている。水は非支持なので、
**睡蓮は唯一置けるはずの水の上で拒否され、石の上に置ける。**

F7 は現在**休眠している**。10 種のどれも `PlaceableItemType` ではないので誰も到達できない。
F7 自身の文はこう予告している —— 「it wakes when a roster row is added by somebody who is
not reading this file」。**この 10 リテラルがまさにその行だった。**

#### 追記: 保留の理由は消えた。残っているのは作業だけである

**直し方は `supportRule` であり、それは実装された**（§4.6.1、§5.3.2）。
mx-gameplay はフォールバックではなく kernel の規則を読むようになり、**F7 は閉じた**。
つまりこの 10 種がアクティブにする経路は、もう「既知の誤り」ではない。

残っているのは機械的な作業だけである —— `ITEM_TYPES` に 10 リテラル、
10 行のレジストリの `DROPS_NOTHING` を既定規則に戻す、両側のドロップテスト。
**それは本作業には含めていない。**リテラルを 1 つ足すと `api-lock.md` が変わり
4 週間の窓がリセットされる（§5 の注意書き）ので、これは「ついでに」やる変更ではない。

それまでは 10 行のレジストリが `drops: DROPS_NOTHING` を**明示的に**述べている ——
残る費用は「1 列における明示された相違」であって、「2 列における嘘」ではない。
