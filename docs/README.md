...
必要な Node.js と pnpm のバージョンは、リポジトリの [package.json](https://github.com/nerima-games/mc-kernel/blob/main/package.json) にある `engines` を参照してください。依存関係をインストールした後、変更の検証は次のコマンドで実行できます。
...
| [versioning.md](./versioning.md) | 互換性と変更管理 |
| [block-position-key-migration.md](./block-position-key-migration.md) | 座標キーの共有化判断 |
